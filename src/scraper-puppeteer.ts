      ...
      const properties = await page.evaluate(() => {
        ...
        for (const selector of selectors) {
          ...
          if (elements.length > 0) {
            // 最初の3件を取得
            const items = Array.from(elements).slice(0, 3).map((el: any) => {
              ...
              // タイトルが取得できない場合の代替手段
              if (!title) {
                const titleEl = el.querySelector('[class*="title"], h2, h3, h4, .bukken-name, .property-name');
                if (titleEl) {
              ...
              // 価格を取得
              const priceEl = el.querySelector('[class*="price"], .bukken-price, .property-price');
              if (priceEl) {
              ...
              // 場所を取得
              const locationEl = el.querySelector('[class*="location"], [class*="address"], .bukken-address');
              if (locationEl) {
