...
export class ComprehensiveMonitorService {
  ...
  private startBotProtectionMonitoring(): void {
    const interval = setInterval(async () => {
      try {
        const metrics = await this.metricsCollector.collectBotProtectionMetrics();
  ...
  private startResponseTimeMonitoring(): void {
    const interval = setInterval(async () => {
      try {
        const metrics = await this.metricsCollector.collectResponseTimeMetrics();
  ...
  private async evaluateBotProtectionMetrics(metrics: BotProtectionMetrics): Promise<void> {
    // 成功率の評価
    ...
    // サイト別成功率の評価
    for (const [site, successRate] of Object.entries(metrics.siteSuccessRates)) {
      if (successRate < 30) {
        await this.alertManager.sendAlert({
          level: 'WARNING',
  ...
  private async evaluateResponseTimeMetrics(metrics: ResponseTimeMetrics): Promise<void> {
    // 平均応答時間の評価
  ...
  private async evaluateProcessMetrics(metrics: ProcessMetrics): Promise<void> {
    ...
    // プロセス生存時間の評価
    const longRunningProcesses = metrics.processLifetimes.filter(lifetime => lifetime > 600000); // 10分
