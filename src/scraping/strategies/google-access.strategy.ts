...
export class GoogleAccessStrategy {
  ...
  async execute(url: string): Promise<ScrapingResult> {
    ...
    try {
      ...
      for (let i = 0; i < searchPatterns.length; i++) {
        try {
          const result = await this.tryGoogleSearchPattern(page, searchPatterns[i], url);
          if (result.success) {
            return {
              ...result,
          ...
          this.logger.warn(`Google search pattern ${i + 1} failed:`, error.message);
  ...
  private async prepareBotDetection(page: Page): Promise<void> {
    ...
    try {
      ...
      this.logger.warn('Bot detection preparation failed, continuing...', error.message);
    }
  }
  ...
  private async tryGoogleSearchPattern(
    ...
    if (!currentUrl.includes(targetDomain)) {
      throw new Error(`Failed to reach target domain. Current URL: ${currentUrl}`);
    }
  ...
  private async simulateHumanClick(page: Page, element: ElementHandle): Promise<void> {
    // 要素の位置を取得
  ...
  private async adaptiveWait(attemptNumber: number): Promise<void> {
    // 指数バックオフ + ランダムジッター
    const delay = this.baseDelay * Math.pow(2, attemptNumber) + Math.random() * 1000;
