...
export class ErrorClassifier {
  static classify(error: Error): ScrapingError {
    ...
    if (message.includes('bot') || message.includes('captcha') || message.includes('403') || message.includes('access denied')) {
    ...
    if (message.includes('browser') || message.includes('page') || message.includes('context')) {
      return new BrowserError(error.message);
    }
    
    if (message.includes('memory') || message.includes('cpu') || message.includes('limit')) {
