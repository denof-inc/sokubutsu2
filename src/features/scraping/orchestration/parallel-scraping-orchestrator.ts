...
export class ParallelScrapingOrchestrator {
  ...
  async executeBatch(tasks: ScrapingTask[]): Promise<BatchResult> {
    ...
    this.logger.log(`Batch execution completed: ${results.length} successful, ${failures.length} failed`);
    this.logger.log(`Total time: ${totalTime}ms, Average: ${averageTime.toFixed(2)}ms`);
  ...
  private async executeParallel(
    ...
    // 最大同時実行数まで並列でタスクを開始
    while (this.taskQueue.length > 0 && this.activeTasks < this.maxConcurrency) {
   ...
   * 実際のスクレイピング処理（簡略版）
   */
  private async performScraping(page: any, task: ScrapingTask): Promise<ScrapingResult> {
    const startTime = Date.now();
