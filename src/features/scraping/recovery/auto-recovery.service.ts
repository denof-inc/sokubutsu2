...
interface RecoveryStrategy {
  name: string;
  execute: (error: ScrapingError, context: RecoveryContext) => Promise<RecoveryResult>;
...
interface RecoveryStats {
  ...
  recoveryStrategies: Record<string, {
    attempts: number;
    successes: number;
    failures: number;
  }>;
...
export class AutoRecoveryService {
  ...
  private initializeStrategies(): void {
    this.strategies.push(
      // Bot検知回復戦略
      {
        ...
        execute: async (error, context) => this.recoverFromBotDetection(error as BotDetectionError, context),
      },
      ...
      // ネットワークエラー回復戦略
      {
        ...
        execute: async (error, context) => this.recoverFromNetworkError(error as NetworkError, context),
      },
      ...
      // タイムアウト回復戦略
      {
        ...
        execute: async (error, context) => this.recoverFromTimeout(error as TimeoutError, context),
      },
      ...
      // ブラウザエラー回復戦略
      {
        ...
        execute: async (error, context) => this.recoverFromBrowserError(error as BrowserError, context),
      },
      ...
      // リソース制限回復戦略
      {
        ...
        execute: async (error, context) => this.recoverFromResourceLimit(error as ResourceLimitError, context),
      },
    );
  }
  ...
  async attemptRecovery(
    ...
    if (!strategy) {
      this.logger.error(`No recovery strategy found for error: ${scrapingError.code}`);
    ...
    try {
      ...
      this.logger.error(`Recovery strategy ${strategy.name} threw error:`, recoveryError);
      this.stats.failedRecoveries++;
      this.stats.recoveryStrategies[strategy.name].failures++;
    ...
    switch (error.detectionType) {
      case 'captcha':
        ...
        return {
          ...
          delay: 30000, // 30秒待機
          message: 'Switching to Google access strategy after CAPTCHA detection',
        };
