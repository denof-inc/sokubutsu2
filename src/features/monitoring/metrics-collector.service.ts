...
export class MetricsCollectorService {
  ...
  private scrapingAttempts: Map<string, { success: number, failure: number }> = new Map();
  ...
  async collectBotProtectionMetrics(): Promise<BotProtectionMetrics> {
    ...
    return {
      overallSuccessRate: totalAttempts > 0 ? (totalSuccess / totalAttempts) * 100 : 0,
      siteSuccessRates,
      totalAttempts,
  ...
  async collectResponseTimeMetrics(): Promise<ResponseTimeMetrics> {
    ...
    return {
      averageResponseTime: validResponseTimes.length > 0 
      ...
      timeoutRate: totalRequests > 0 ? (this.timeouts / totalRequests) * 100 : 0,
      totalRequests,
      timestamp: new Date()
    };
  }
  ...
  async collectProcessMetrics(): Promise<ProcessMetrics> {
    ...
    const now = Date.now();
    const processLifetimes = processInfo.map(info => 
      now - info.createdAt.getTime()
    );
    ...
    return {
      ...
      processLifetimes,
      averageLifetime: processLifetimes.length > 0 
        ? processLifetimes.reduce((a, b) => a + b, 0) / processLifetimes.length 
        : 0,
      timestamp: new Date()
    };
  }
  ...
  private async getDiskUsage(): Promise<{ usagePercent: number; free: number; total: number } | undefined> {
    // ディスク使用量の取得（簡易版）
