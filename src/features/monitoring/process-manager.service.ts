...
export class ProcessManagerService {
  ...
  async createBrowser(sessionId: string, maxLifetime: number = 300000): Promise<Browser> {
    try {
      ...
      const timeout = setTimeout(async () => {
        await this.closeBrowser(sessionId, 'TIMEOUT');
      }, maxLifetime);
  ...
  async closeBrowser(sessionId: string, reason: string = 'MANUAL'): Promise<void> {
    ...
    if (browser) {
      try {
        ...
        this.logger.error(`ブラウザプロセス終了失敗: ${sessionId} - ${error.message}`);
      }
