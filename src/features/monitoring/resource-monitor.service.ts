...
export class ResourceMonitorService {
  ...
  private async evaluateMetrics(metrics: SystemMetrics): Promise<void> {
    ...
    if (metrics.memory.usagePercent > 80) {
      this.logger.error(`メモリ使用量が危険レベル: ${metrics.memory.usagePercent.toFixed(2)}%`);
      await this.triggerEmergencyCleanup();
    } else if (metrics.memory.usagePercent > 60) {
      this.logger.warn(`メモリ使用量が高レベル: ${metrics.memory.usagePercent.toFixed(2)}%`);
    }
    
    // ヒープメモリの監視
    if (metrics.memory.heapUsed > 500 * 1024 * 1024) { // 500MB
      this.logger.warn(`ヒープメモリ使用量が高レベル: ${(metrics.memory.heapUsed / 1024 / 1024).toFixed(2)}MB`);
    }
  }
